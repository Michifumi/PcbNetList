...
Ａ.概要
 ...
 そのサブプログラムを使って、OrCADのネットリストファイルを入力し、KiCADv4の基板設計ソフトＰｃｂｎｅｗ用ネットリストを出力できるにプログラムを提供する。
...
　それは、上記4.の２行目を
  net = netlist.Calay_Read('「OrCAD出力ネットリストファイル名」','「部番と部品パッド情報をcsvファイル名」')
 へ変更して実行すると、部品パッドを変更したネットリストファイルが生成される。
 尚、「部番と部品パッド情報をcsvファイル名」は、"01_DESIGN2.csv"を参考にして作成してください。
